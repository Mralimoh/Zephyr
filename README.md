...
۱. **فعال‌سازی API:** به [Google Cloud Console](https://console.cloud.google.com/) بروید، یک پروژه بسازید و از بخش **Library**، سرویس **Google Drive API** را جستجو و فعال کنید.

۲. **تنظیم صفحه رضایت (Consent Screen):**

۲.۱. به بخش **APIs & Services** و سپس **OAuth consent screen** بروید.

۲.۲. نوع کاربر را **External** انتخاب کنید. نام برنامه، ایمیل پشتیبانی و ایمیل برنامه‌نویس را وارد کرده و ذخیره کنید.

۲.۳. **بسیار مهم:** در انتهای کار در همان صفحه، روی دکمه **Publish App** کلیک کنید تا وضعیت از Testing به In Production تغییر کند. در غیر این صورت، توکن شما هر ۷ روز منقضی شده و ارتباط قطع می‌شود.

۳. **ساخت اعتبارنامه (Credentials):**

۳.۱. به بخش **APIs & Services** و سپس **Credentials** بروید.

۳.۲. روی **Create Credentials** کلیک کرده و **OAuth client ID** را انتخاب کنید.

۳.۳. نوع برنامه (**Application type**) را روی **Desktop App** تنظیم و تایید کنید.

۴. **دانلود و آماده‌سازی:**

۴.۱. فایل JSON ساخته شده را دانلود کنید.

۴.۲. نام فایل را دقیقاً به `credentials.json` تغییر داده و در پوشه اصلی پروژه قرار دهید.---
